# This script will eventually report on how closely copies of individual 
#    words are spaced in the input running text.
...
#    last_seen is table of tuples with most recent position first
...
def find_repeats(line,last_seen,seen_repeated,seen_total,k,linestart,window,required_num):
    repeats = []
...
def process_file_simple (infile, k,window,outfile,outfile2,numrequired):
    ...
    seen_total = Counter()   # count of how often each tuple has been seen in this file
    ...
    with open(infile) as infi:
        for rawline in infi:
            ccc += 1
            if (ccc%1000 == 0): print(".", file=sys.stderr,end='',flush=True)
            line = rawline.strip()
            print(f">>>WORDS: {linestart}   {line} \n")
            line_no_blanks = remove_whitespace(line)
            print(f">>>INPUT: {linestart}  {line_no_blanks} \n")
#            show_repeats(line_no_blanks,last_seen,k,linestart,args.window)
            repeats = find_repeats(line_no_blanks,last_seen,seen_repeated,seen_total,k,linestart,
                                   window,numrequired)
            print(repeats)
            print("\n")
    ...
    with open(outfile,"w") as outf:
        for mytuple in last_seen:
            print(f"{mytuple} {seen_repeated[mytuple]} {seen_total[mytuple]} {len(mytuple)}", file=outf)
